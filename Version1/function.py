...
def display_assistant_message(message):
    ...
    for message in reversed(message):
            if message.role in ["user","assistant"]:
                with st.chat_message(message.role):
                    for content in message.content:
                        if message.role == "user" and len(content.text.value) > 500:
                            continue
                        else:
                            if content.type == "text":
                                st.markdown(content.text.value)
...
def chat_prompt(client, assistant_option, thread_id, messages):
    if prompt := st.chat_input("Enter your message here"):
        with st.chat_message("user"):
            st.markdown(prompt)
        st.session_state.messages = messages.append(client.beta.threads.messages.create(
            thread_id=thread_id,
            role="user",
            content=prompt,
        ))
        ...
        if st.session_state.run.status == "completed": 
            st.empty()
            display_assistant_message(st.session_state.messages)
